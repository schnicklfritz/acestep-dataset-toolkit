...
class SpatialPipelineWorker(QThread):
    ...
    def _caption_chunks(self, chunks):
        ...
        self.progress.emit(48, "Waiting for the Kaggle dataset to finish processing…")
        ready_reason = []
        ...
        #    with this clip. The old schema-less default ("Write 3 to 5 sentences.
        #    Start with A or An") is ignored -- see caption_spec.
        ...
        kernel_script = (
            ...
            # structural sections, so windowing inside the kernel would cut each
            # section a second time. Whole-song windowing belongs to the caption
            # kernel's own run path (workers/caption.py).
