"""Background worker: DSP normalization (EBU R128 loudness + sample rate).

Extracted from dataset_manager.py. Normalizes each sample's audio to a
target LUFS/sample-rate via ffmpeg's loudnorm filter, backing up the
original file first. Falls back to a plain copy if ffmpeg fails, so a
normalized path always exists afterward.
"""
import os
import shutil
import subprocess
    ...
    def run(self):
        try:
            ...
            for idx, s in enumerate(self.samples):
                ...
                fname = s.get("filename") or f"sample_{sid}.wav"
                ...
                self.progress.emit(
                    int(100 * idx / total), f"Normalizing ({self.target_lufs} LUFS): {fname}"
                )

                cmd = [
                    "ffmpeg", "-y", "-i", orig_path,
                    "-af", f"loudnorm=I={self.target_lufs}:TP=-1.0:LRA=11",
                    "-ar", str(self.target_sr), "-ac", "2", norm_path,
                ]
                res = subprocess.run(cmd, capture_output=True, text=True, check=False)
                if res.returncode == 0 and os.path.exists(norm_path):
                    self.file_normalized.emit(
                        sid, backup_path, norm_path, self.target_sr, self.target_lufs
                    )
                else:
